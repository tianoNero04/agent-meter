...
- 无固定轮询：刷新只由弹窗出现、手动刷新和目录变化（1 秒防抖后的本地刷新）触发。
- 刷新任务可取消，集中在 `RefreshCoordinator` 管理。
- 日志采集在 utility 优先级后台执行，结果回主 actor 发布。
- 采集器按文件做增量缓存（修改时间 + 文件大小为键），未变化的文件不重复解析；解析按行流式读取，并逐行释放 autorelease 对象，避免全量日志堆积内存。
- 内存中的历史快照与落盘保持同一 30 天裁剪规则，常驻期间不无限增长。
- 弹窗关闭后不保持网络连接，无 CLI 常驻进程；`codex app-server` 只在账号刷新路径短暂启动。
